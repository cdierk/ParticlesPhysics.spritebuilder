...
- (bool)isBluetoothLESupported
{
    ...
    if ([viewController respondsToSelector:@selector(shouldDisplayAlertTitled:messageBody:)]) {
        [viewController shouldDisplayAlertTitled:@"Bluetooth LE Support" messageBody:message];
    }
...
- (void) rangeTick:(NSTimer*)timer
{
    ...
    if (update) {
        if (didUpdateDiscoveredRFduinoFlag) {
            ScanViewController *viewController = [ScanViewController alloc];
            [viewController didUpdateDiscoveredRFduino:nil];
        }
    }
}
...
- (void)centralManager:(CBCentralManager *)central didConnectPeripheral:(CBPeripheral *)peripheral
{
    ...
    if (rfduino) {
        [rfduino connected];
        ScanViewController *viewController = [ScanViewController alloc];
        [viewController didConnectRFduino:rfduino];
    }
}
...
- (void)centralManager:(CBCentralManager *)central didDisconnectPeripheral:(CBPeripheral *)peripheral error:(NSError *)error
{
    ...
    void (^block)(void) = ^{
        ScanViewController *viewController = [ScanViewController alloc];
        if ([viewController respondsToSelector:@selector(didDisconnectRFduino:)]) {
            RFduino *rfduino = [self rfduinoForPeripheral:peripheral];
            if (rfduino) {
                [viewController didDisconnectRFduino:rfduino];
            }
        }
    };
...
- (void)centralManager:(CBCentralManager *)central didDiscoverPeripheral:(CBPeripheral *)peripheral advertisementData:(NSDictionary *)advertisementData RSSI:(NSNumber *)RSSI
{
    ...
    if (added) {
        ...
        if (didUpdateDiscoveredRFduinoFlag) {
            ScanViewController *viewController = [ScanViewController alloc];
            [viewController didUpdateDiscoveredRFduino:rfduino];
        }
    }
}
...
- (void)centralManager:(CBCentralManager *)central didFailToConnectPeripheral:(CBPeripheral *)peripheral error:(NSError *)error
{
    ...
    ScanViewController *viewController = [ScanViewController alloc];
    if ([viewController respondsToSelector:@selector(shouldDisplayAlertTitled:messageBody:)]) {
        [viewController shouldDisplayAlertTitled:@"Connect Failed" messageBody:error.description];
    }
}
...
- (void)startScan
{
    ...
    ScanViewController *viewController = [ScanViewController alloc];
    didUpdateDiscoveredRFduinoFlag = [viewController respondsToSelector:@selector(didUpdateDiscoveredRFduino:)];
...
{
    ScanViewController *viewController = [ScanViewController alloc];
    if ([viewController respondsToSelector:@selector(didLoadServiceRFduino:)]) {
        [viewController didLoadServiceRFduino:rfduino];
    }
}
